...
const DEFAULT_AVATAR =
  "https://ui-avatars.com/api/?name=MP&background=e5e7eb&color=374151";
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    async function loadNextChunk() {
      ...
      // âœ… set total count ONCE (for stable pagination)
      ...
      // â±ï¸ controlled background loading
  ...
  const visibleMembers = useMemo(() => {
    return members.slice(0, page * ITEMS_PER_PAGE);
  }, [members, page]);

 
  const processedMembers = useMemo(() => {
    let data = [...visibleMembers];
    ...
    // House filter
    if (house !== "all") {
      data = data.filter((m) => m.house.toLowerCase().includes(house));
    }
    ...
    return data;
  }, [visibleMembers, query, house, sortBy]);

  const totalPages = Math.ceil(
    (totalCount ?? processedMembers.length) / ITEMS_PER_PAGE,
  );
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold text-ink-gray">
            ...
            <span className="text-gray-400 font-normal ml-2">
              ({totalCount ?? processedMembers.length} Members)
            </span>
          </h2>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="seniority" disabled>
                Seniority (Coming Soon)
              </option>
            </select>
          </div>
        </div>
